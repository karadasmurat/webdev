...
// return a set of n cities
function getCities(n) {

    const citySet = new Set();

    while (citySet.size < n) {
        let cityIndex = Math.floor((Math.random() * cities.length));
        citySet.add(cities[cityIndex]);
    }

    return citySet;
}

// return a set of n titles, combinations of descriptions and places arrays
function getTitles(n) {

    const nameSet = new Set();

    while (nameSet.size < n) {
        let descIndex = Math.floor((Math.random() * descriptors.length));
        let placeIndex = Math.floor((Math.random() * places.length));
        nameSet.add(descriptors[descIndex] + " " + places[placeIndex]);
    }
